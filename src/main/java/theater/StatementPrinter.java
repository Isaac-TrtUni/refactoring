        ...
        for (Performance perf : invoice.getPerformances()) {
            ...
            switch (play.getType()) {
                ...
                case "history":
                    ...
                    if (perf.getAudience() > Constants.HISTORY_AUDIENCE_THRESHOLD) {
                        thisAmount += Constants.HISTORY_OVER_BASE_CAPACITY_PER_PERSON
                                * (perf.getAudience() - Constants.HISTORY_AUDIENCE_THRESHOLD);
                    }
                    break;
                case "pastoral":
                    thisAmount = Constants.PASTORAL_BASE_AMOUNT;
                    if (perf.getAudience() > Constants.PASTORAL_AUDIENCE_THRESHOLD) {
                        thisAmount += Constants.PASTORAL_OVER_BASE_CAPACITY_PER_PERSON
                                * (perf.getAudience() - Constants.PASTORAL_AUDIENCE_THRESHOLD);
                    }
                    break;
                default:
            ...
            // add volume credits
            volumeCredits += Math.max(perf.getAudience() - Constants.BASE_VOLUME_CREDIT_THRESHOLD, 0);
            ...
            result.append(String.format("  %s: %s (%s seats)%n", play.getName(),
                    format.format(thisAmount / Constants.PERCENT_FACTOR), perf.getAudience()));
